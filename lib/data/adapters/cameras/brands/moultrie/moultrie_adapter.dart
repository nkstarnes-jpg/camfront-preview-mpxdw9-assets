...
/// Moultrie v1 [CameraAdapter] — **photo-dump / filesystem import**.
///
/// No public official Moultrie API was found. Cloud OAuth and email/FTP are
/// stubbed as TODO; see `README.md` in this folder.
class MoultrieAdapter implements CameraAdapter {
  MoultrieAdapter({
    PhotoDumpSource? photoDumpSource,
  }) : _photoDumpSource =
            photoDumpSource ?? const FilesystemPhotoDumpSource();

  static const String brandIdValue = 'moultrie';
  ...
  Future<void> connect(CameraCredentials creds) async {
    if (creds.brandId != brandIdValue) {
      throw CameraAdapterError(
        'MoultrieAdapter expected brandId "$brandIdValue", got "${creds.brandId}"',
      );
    }
    ...
    if (path == null || path.isEmpty) {
      throw const CameraAdapterError(
        'Moultrie v1 requires extra["photoDumpPath"]. '
  ...
  void _ensureConnected() {
    if (!_connected || _photoDumpPath == null) {
      throw const CameraAdapterError('MoultrieAdapter is not connected');
    }
  }
  ...
  static String _remoteIdFor(PhotoDumpEntry entry) =>
      'moultrie-dump:${entry.absolutePath}';
